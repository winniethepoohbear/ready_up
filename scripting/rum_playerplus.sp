...
public KickSurvivorBots() {
	...
	if (TotalHumanCount() > 0 && TotalSurvivorCount() <= 4) return;
	for (new i = 1; i <= MaxClients; i++) {
...
public TotalHumanCount() {
	...
	for (new i = 1; i <= MaxClients; i++) {
		...
		if (IsClientInGame(i) && GetClientTeam(i) == TEAM_SURVIVOR && !IsFakeClient(i)) Count++;
	}
	return Count;
}
...
public FindSurvivorBot() {
	...
	for (new i = 1; i <= MaxClients; i++) {
		...
		if (IsClientConnected(i) && IsClientActual(i) && IsClientInGame(i) && GetClientTeam(i) == TEAM_SURVIVOR && IsFakeClient(i)) return i;
	}
