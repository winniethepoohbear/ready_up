		...
		switch (param2)
		{
			case 1: {
				...
				if (GetClientTeam(client) != TEAM_SPECTATOR) {
					...
					if (HasCommandAccess(client, "a") || HasCommandAccess(client, "k") || HasCommandAccess(client, "z")) {
...
{
	if (IsClientActual(client)) FindAndSetSurvivorModel(client);
	return Plugin_Stop;
}
...
stock GetModelCount(client, String:Model[])
{
	...
	for (new i = 1; i <= MaxClients; i++)
	{
		if (IsClientHuman(i) && GetClientTeam(i) == TEAM_SURVIVOR)
		{
